...
os.system("clear")
autoketik(f"{biru}[{kuning}Warning{biru}] {W}This Tool Has Made By whysixmift, follow github ya dik")
time.sleep(3)
os.system("xdg-open https://github.com/whysixmift")
autoketik(f"{biru}[{kuning}Warning{biru}] {W}Thx yang udah follow github, semoga work ya wir")
time.sleep(3)
os.system("clear")
autoketik(f"""
...
{abu}-----------------------------------------
{putih}[{biru}•{putih}] {biru}Author {putih}   : whysixmift
{putih}[{biru}•{putih}] {abu}GitHub {putih}   : whysixmift
{putih}[{biru}•{putih}] {ungu}Instagram {putih}: @whysixmift
{W}[{Y}•{W}] IP Address Kamu {putih}  :{Y} {ip}
{W}[{Y}•{W}] Waktu/Jam {putih}:{Y} {localtime}
{W}[{Y}•{W}] Total Runtime {putih}:{Y} {getvisit['value']}
""")

nomor = input(f"{W}[{R}• {kuning}•{hijau}•{W}] {biru}Nomor Target (gunakan dengan baik, kesalahan ditanggung pengguna) {W}: ")
